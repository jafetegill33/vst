class VikingSettlementTycoon {
    ...
    getDayNightInfo() {
        ...
        const sunAngle = cycleProgress * Math.PI * 2 - Math.PI; // -π to π
        
        // Day phases: 0-0.25: Night, 0.25-0.5: Dawn/Morning, 0.5-0.75: Day, 0.75-1: Dusk/Evening
        let phase = 'day';
        let lightLevel = 1.0;
        ...
        if (cycleProgress < 0.25) {
            ...
            phase = 'dawn';
            lightLevel = 0.5 + (cycleProgress - 0.25) * 5; // 0.5 to 1.0
        } else if (cycleProgress < 0.65) {
            phase = 'day';
            lightLevel = 1.0;
        } else if (cycleProgress < 0.75) {
            phase = 'dusk';
            lightLevel = 1.0 - (cycleProgress - 0.65) * 5; // 1.0 to 0.5
        } else {
            phase = 'night';
            lightLevel = 0.5 - (cycleProgress - 0.75) * 0.8; // 0.5 to 0.3
        }
        
        // Sun position for ray casting
        ...
        switch (phase) {
            case 'dawn':
                ambientColor = `rgba(255, 180, 120, ${lightLevel * 0.1})`;
                sunColor = `rgba(255, 200, 100, ${lightLevel * 0.3})`;
                break;
            case 'day':
                ambientColor = `rgba(255, 255, 220, ${lightLevel * 0.05})`;
                sunColor = `rgba(255, 255, 200, ${lightLevel * 0.2})`;
                break;
            case 'dusk':
                ambientColor = `rgba(255, 120, 80, ${lightLevel * 0.15})`;
                sunColor = `rgba(255, 150, 80, ${lightLevel * 0.4})`;
                break;
            case 'night':
                ambientColor = `rgba(80, 80, 150, ${0.2 - lightLevel * 0.1})`;
                sunColor = `rgba(180, 180, 255, ${0.1})`;
                break;
            default:
                ambientColor = `rgba(255, 255, 255, 0.05)`;
                sunColor = `rgba(255, 255, 255, 0.1)`;
        }
